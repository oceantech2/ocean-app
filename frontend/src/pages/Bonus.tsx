...
type AbaRemuneracao = 'bonus' | 'comissao';
...
export default function BonusPage() {
  ...
  const [aba, setAba] = useState<AbaRemuneracao>('comissao');
  ...
  const carregarBonus = async () => {
    try {
      setLoading(true);
      const res = await bonusService.listar(
        0,
        500,
        bonusColaboradorId ? Number(bonusColaboradorId) : undefined,
        undefined,
        bonusAno || undefined,
        undefined,
        aba,
      ...
      toast.error(eBonus ? 'Erro ao carregar bônus' : 'Erro ao carregar comissões');
    } finally {
      setLoading(false);
    }
  };
  ...
  const trocarAba = (next: AbaRemuneracao) => {
    if (next === aba) return;
    setAba(next);
    setPagina(0);
    setSelecionados(new Set());
  ...
  const liberar = async (b: Bonus) => {
    if (!confirm(`Liberar ${rotulo} de ${fmt(b.valor_bonus)}?`)) return;
    try {
      await bonusService.liberar(b.id);
      toast.success(eBonus ? 'Bônus liberado' : 'Comissão liberada');
      carregarBonus();
    } catch (e: any) {
  ...
  const pagar = async (b: Bonus) => {
    if (!confirm(`Marcar como pago ${rotulo} de ${fmt(b.valor_bonus)}?`)) return;
    try {
      await bonusService.pagar(b.id);
      toast.success(eBonus ? 'Bônus pago' : 'Comissão paga');
      carregarBonus();
    } catch (e: any) {
  ...
  const exportar = () => {
    if (eBonus) {
      exportarCSV(bonusFiltrado.map((b) => ({
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        ...
        <ResponsiveContainer width="100%" height={250}>
          <BarChart data={graficoDados}>
            ...
            <Legend />
            <Bar dataKey="total" name={eBonus ? 'Total Bônus' : 'Total Comissões'} fill="#10B981" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
